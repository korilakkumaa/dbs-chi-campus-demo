...
export function scoresYearLabel(startYear: number): string {
  return formatAcademicYearLabel(startYear)
}

export function hasImportedScoresForYear(startYear: number): boolean {
  return (SCORES_IMPORTED_ACADEMIC_YEARS as readonly number[]).includes(
    startYear,
  )
}
